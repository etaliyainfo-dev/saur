...
1. From project root, run:
   ...
   ```
2. This creates `meeshomart-installable.zip` in the project root.
3. In WordPress admin: **Appearance → Themes → Add New → Upload Theme**.
4. Upload `meeshomart-installable.zip` (do **not** upload the full repository ZIP like `saur-xxxx.zip`).
...
## Troubleshooting
- **Theme upload failed / stylesheet missing:** This happens when repo ZIP is uploaded. Generate and upload `meeshomart-installable.zip` via `bash meeshomart/package-theme.sh`.
