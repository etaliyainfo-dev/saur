(() => {
  ...
  const normalizeId = (text) =>
    clean(text).replace(/^act_/i, "").replace(/\D/g, "");
  ...
  const visibleLines = (el = document.body) =>
    String(el?.innerText || "")
      .split(/\n+/)
      .map(clean)
      .filter(Boolean);
  ...
  function hasDropdownText(el) {
    ...
    return (
      text.includes("business portfolios") &&
      text.includes("ad accounts") &&
      /ad account id\s*:/i.test(text)
    );
  }
  ...
  function findDropdownRoot() {
    const candidates = [
      ...document.querySelectorAll(
        '[role="dialog"], [role="menu"], [role="listbox"], div, section',
      ),
    ]
      .filter(isVisible)
  ...
  function nameFromBusinessText(text) {
    const lines = String(text || "")
      ...
      .map(clean)
      .filter(Boolean);
    const countIndex = lines.findIndex((line) =>
      /\b\d+\s+ad accounts?\b/i.test(line),
    );
    if (countIndex > 0) return lines[countIndex - 1];
    return clean(lines[0] || "")
      .replace(/\b\d+\s+ad accounts?.*$/i, "")
      .trim();
  }
  ...
  function elementLooksLikeBusinessRow(el, rootRect) {
    ...
    if (/^ad accounts$/i.test(text) || /^business portfolios$/i.test(text))
      return false;
    const rect = el.getBoundingClientRect();
  ...
  function collectVisiblePortfolioRows(root) {
    ...
    const rowCandidates = [
      ...root.querySelectorAll(
        '[role="button"], [role="option"], [tabindex], button, div',
      ),
    ]
      .filter(isVisible)
    ...
    for (const el of rowCandidates) {
      ...
      rows.push({
        ...
        accountCountText,
        isSelected:
          /selected/i.test(text) || el.getAttribute("aria-selected") === "true",
        element: el,
      });
    }
    return rows;
  ...
  function findPortfolioRowByName(root, row) {
    return (
      collectVisiblePortfolioRows(root).find(
        (candidate) =>
          candidate.name === row.name &&
          candidate.accountCountText === row.accountCountText,
      )?.element || row.element
    );
  }
  ...
  function collectRowsAfterGentleScroll(root) {
    ...
    const seen = new Map(
      rows.map((row) => [
        `${comparable(row.name)}|${row.accountCountText}`,
        row,
      ]),
    );
    ...
    collectVisiblePortfolioRows(root).forEach((row) =>
      seen.set(`${comparable(row.name)}|${row.accountCountText}`, row),
    );
  ...
  function findPanelCandidates(root) {
    const rootRect = root.getBoundingClientRect();
    return [
      ...root.querySelectorAll('section, [role="region"], [role="main"], div'),
    ]
      .filter(isVisible)
      .filter((el) => {
        const text = visibleText(el);
        if (!/ad account id\s*:/i.test(text) || !/\bad accounts\b/i.test(text))
          return false;
        const rect = el.getBoundingClientRect();
        return (
          rect.left >= rootRect.left + rootRect.width * 0.35 &&
          rect.width > 180 &&
          rect.height > 120
        );
      })
  ...
  function detectPanelHeading(panel) {
    ...
    for (let i = beforeAd.length - 1; i >= 0; i -= 1) {
      ...
      if (
        !line ||
        /^business portfolios$/i.test(line) ||
        /\b\d+\s+ad accounts?\b/i.test(line) ||
        /selected/i.test(line)
      )
        continue;
      return line;
    }
    return (
      beforeAd.find((line) => line && !/\b\d+\s+ad accounts?\b/i.test(line)) ||
      ""
    );
  }
  ...
  function cleanAccountName(rawName, businessName = "") {
    let name = clean(rawName)
      .replace(/\bAd account ID\s*:?\s*(act_)?[0-9\s-]*/gi, "")
      .replace(/\b(New|Saved|Current|Selected)\b/gi, "")
      .replace(/^[•\-–—|]+|[•\-–—|]+$/g, "")
      .trim();
    if (businessName && name.endsWith(`• ${businessName}`))
      name = name.slice(0, -`• ${businessName}`.length).trim();
    return name;
  }
    ...
    for (let i = 0; i < slice.length; i += 1) {
      ...
      if (!name) {
        for (let j = i - 1; j >= Math.max(0, i - 5); j -= 1) {
          ...
          if (
            !candidate ||
            /^ad accounts$/i.test(candidate) ||
            /ad account id\s*:/i.test(candidate) ||
            /^\d+$/.test(candidate) ||
            /\b\d+\s+ad accounts?\b/i.test(candidate)
          )
            continue;
          name = candidate;
          break;
      ...
      accounts.push({
        name: name || `Ad Account ${accountId}`,
        accountId,
        isSelected: /selected|current/i.test(
          `${slice[i - 2] || ""} ${slice[i - 1] || ""} ${slice[i]} ${slice[i + 1] || ""}`,
        ),
      });
    }
    const seen = new Set();
  ...
  async function waitForPanelMatch(root, portfolioName) {
    ...
    for (let i = 0; i < 10; i += 1) {
      ...
      if (similarName(heading, portfolioName))
        return { panel, heading, matched: true };
      await sleep(150);
    }
  ...
  async function scanSelectedPortfolio(root) {
    ...
    const heading = detectPanelHeading(panel);
    const portfolioName =
      heading || selected?.name || "Selected Business Portfolio";
    const accountCountText =
      selected?.accountCountText || accountCountFromText(visibleText(panel));
    const accounts = extractAccountsFromPanel(
      panel,
    ...
    if (selected && heading && !similarName(heading, selected.name)) {
      warnings.push(
        "Business mapping may be approximate because Meta UI only renders the selected portfolio accounts.",
      );
    }
    return {
      businesses: [
        { name: portfolioName, accountCountText, isSelected: true, accounts },
      ],
      warnings,
    };
  }
  ...
  async function scanAllVisiblePortfolios(root) {
    ...
    if (!rows.length)
      return {
        businesses: [],
        warnings: ["No visible business portfolio rows were found."],
      };
    ...
    for (const row of rows) {
      ...
      try {
        ...
        if (!match.matched) {
          console.log(
            "[Meta Ads Switcher] Retrying portfolio click:",
            row.name,
          );
        ...
        businesses.push({
          name: row.name,
          accountCountText: row.accountCountText,
          isSelected: row.name === original.name,
          accounts,
        });
      } catch (error) {
        const reason = `Skipped ${row.name} because the ad account panel did not update.`;
    ...
    if (warnings.length)
      warnings.unshift(
        "Some portfolios were skipped because Meta did not update the account panel fast enough. Open dropdown and rescan.",
      );
  ...
  async function scanDropdown(options = {}) {
    ...
    const result =
      options.mode === "auto"
        ? await scanAllVisiblePortfolios(root)
        : await scanSelectedPortfolio(root);
    const businesses = (result.businesses || []).filter(
      (business) => (business.accounts || []).length,
    );
  ...
  chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    scanDropdown({ mode: message.mode === "auto" ? "auto" : "selected" })
      .then(sendResponse)
      .catch((error) =>
        sendResponse({
          success: false,
          error: error.message || "Could not scan Ads Manager dropdown.",
        }),
      );
    return true;
  });
